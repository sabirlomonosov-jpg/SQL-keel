...
--
-- Host: 127.0.0.1
-- Loomise aeg: Märts 05, 2026 kell 04:30 PL
...
-- Toimingud
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `filmSuurem` (IN `arv` INT)  COMMENT 'filmipikkus, mis pikkus on suure, kui @arv' BEGIN
DECLARE today TIMESTAMP DEFAULT CURRENT_DATE;
SELECT filmNimetus, pikkus, today
FROM film
WHERE pikkus > arv;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `hinnaKategooria` (IN `odavPiir` INT, IN `keskPiir` INT)   BEGIN
	SELECT pilethind , IF (pilethind < odavPiir, 'soodne',
    IF (pilethind > keskPiir, 'standaartne', 'kallis')) as
    hinnakategooria from kinokava;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `keeleliisamine` (IN `keel` VARCHAR(30), IN `KeelInglise` VARCHAR(30), IN `KeelVene` VARCHAR(30))   BEGIN
	INSERT INTO keel(keelNimi, keelNimiInglise, keelNimiVene)
    VALUES(keel, KeelInglise, KeelVene);
    SELECT *FROM keel;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `kustutsZanr` (IN `id` INT)  COMMENT 'ID järgi žanr-tabelis olevate kirjete kustutamiseks' BEGIN
SELECT * FROM zanr;
DELETE FROM zanr 
WHERE zanrID=id;
SELECT * FROM zanr;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `tervist` ()  DETERMINISTIC COMMENT 'A prcedure' BEGIN
 	SELECT 'Hello World !';
END$$
